...
class Alphabet:
    ...
    def __len__(
        self,
    ):  # implements the "len" operator, e.g. "len(Alphabet('XYZ'))" results in 3
        return len(
            self.symbols
        )  # will tell you the length of the symbols in an Alphabet instance

    def __contains__(
        self, sym
    ):  # implements the "in" operator, e.g. "'A' in Alphabet('ACGT')" results in True
        return (
            sym in self.symbols
        )  # will tell you if 'A' is in the symbols in an Alphabet instance

    def __iter__(
        self,
    ):  # method that allows us to iterate over all symbols, e.g. "for sym in Alphabet('ACGT'): print sym" prints A, C, G and T on separate lines
...
class Sequence:
    ...
    def __getitem__(
        self, ndx
    ):  # [ndx] operator (retrieve a specified index (or a "slice" of indices) of the sequence data.
    ...
    for line in string.splitlines():  # read every line
        ...
        if line[0] == ">":  # start of new sequence
            ...
            if len(parts) > 0:
                seqname = parts[0]
            if len(parts) > 1:
                seqannot = line[len(seqname) + 2 :]  # the rest of the line
            seqdata = []
...
class GeneProfile:
    ...
    def __init__(self, dataset_name="", sample_names=[], profiles=None):
        ...
        self.genes = (
            profiles or {}
        )  # dictionary for storing all gene--measurement pairs
    ...
    if dbName.startswith("uniprot"):
        ...
        if format == "list":
            if size == None:  # no limit to number of results returned
                url = "{}{}/stream?format={}&query={}".format(
                    __uniprotUrl__, dbName, format, urllib.parse.quote(query, safe='"')
                )
            else:
                url = "{}{}/stream?format={}&query={}&size={}".format(
                    __uniprotUrl__,
                    dbName,
                    format,
...
def getSequence(
    query, id=None, dbName="uniprotkb", alphabet=Protein_Alphabet, format="fasta"
):
    ...
    if query:
        url = "{}{}/stream?format={}&query={}".format(
            __uniprotUrl__, dbName, format, query
        )
    elif id:
        url = "{}{}/stream?format={}&query=accession:{}".format(
            __uniprotUrl__, dbName, format, query
        )
...
def idmap(identifiers, frm="ACC", to="P_REFSEQ_AC"):
    ...
    params = {"from": frm, "to": to, "format": "tab", "query": query}
    if len(query) > 0:
...
def getGenes(goterms, db="UniProtKB", taxo=None):
    ...
    if taxo == None:
        ...
        uri = (
            "http://www.ebi.ac.uk/QuickGO/GAnnotation?format=tsv&db="
            + db
            + "&tax="
            + str(taxo)
            + "&term="
    ...
    def _backwardParsimony(self, aln, seq=None):
        ...
        if self.sequence == None:  # no sequence has been assigned
            ...
            self.left._backwardParsimony(
                aln, Sequence(leftbuf, aln.alphabet, self.label, gappy=True)
            )
            self.right._backwardParsimony(
                aln, Sequence(rightbuf, aln.alphabet, self.label, gappy=True)
            )
...
def runUPGMA(aln, measure, absoluteDistances=False):
    ...
    while (
        len(N) > 1
    ):  # N will contain all "live" clusters, to be reduced to a signle below
        closest_pair = (
            None,
            None,
